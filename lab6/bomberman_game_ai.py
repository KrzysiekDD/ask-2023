...
from bomberman_objects import PlayerPixmapItem, UnbreakableWall, OneBombWall, TwoBombWall, BombPixmapItem, \
    ...
    BackgroundPixmapItem, Board, ExplosionPixmapItem                                                                                                  ;LEFT = 16777234; RIGHT = 16777236; UP = 16777235; DOWN = 16777237; SPACE= 32
from utils import print_graph_to_stdo
import torch
# from bomberman_dqn import QNetwork
import json
import sqlite3
...
def cust_key(key: int):
    type_ = QKeyEvent.KeyPress
    modifiers = Qt.NoModifier
    text = ''
    autorep = False
    count = 1
    return QKeyEvent(type_, key, modifiers, text, autorep, count)
...
class ReplayWidget(QWidget):
    def __init__(self, board_width: int, board_height: int, unbreakable_wall_density: float,
        ...
        super().__init__()
        self.showMaximized()
        self.setWindowTitle("AI Agent playing")
...
class GameScene(QGraphicsScene):
    def __init__(self, board_width: int, board_height: int, unbreakable_wall_density: float,
        ...
        """
        super().__init__()
        self.board_width: int = board_width                                                                                                                                      ;self.player_moves_list = [cust_key(DOWN), cust_key(RIGHT), cust_key(RIGHT), cust_key(UP), cust_key(SPACE), cust_key(DOWN), cust_key(LEFT), cust_key(DOWN), cust_key(RIGHT), cust_key(RIGHT), cust_key(SPACE), cust_key(DOWN), cust_key(LEFT), cust_key(DOWN), cust_key(DOWN), cust_key(SPACE), cust_key(UP), cust_key(RIGHT), cust_key(SPACE), cust_key(LEFT), cust_key(DOWN), cust_key(RIGHT), cust_key(UP), cust_key(LEFT), cust_key(SPACE), cust_key(RIGHT), cust_key(SPACE), cust_key(DOWN), cust_key(LEFT), cust_key(DOWN), cust_key(DOWN), cust_key(DOWN), cust_key(DOWN), cust_key(DOWN), cust_key(LEFT), cust_key(DOWN), cust_key(DOWN), cust_key(SPACE), cust_key(UP), cust_key(LEFT), cust_key(DOWN), cust_key(DOWN), cust_key(RIGHT), cust_key(SPACE), cust_key(UP), cust_key(LEFT), cust_key(UP), cust_key(RIGHT), cust_key(SPACE), cust_key(LEFT), cust_key(DOWN), cust_key(RIGHT), cust_key(UP)]
        self.board_height: int = board_height
        ...
        self.setSceneRect(0, 0, 1280, 1280)
        self.model = torch.load("bomberman_dqn.pt")
        self.player_move_list= generate_actions(self.model)
    ...
    def update_scene(self) -> None:
        ...
        event = self.player_moves_list[self.i]
...
state_size = (20, 20)                                                                                                                              ;QNetwork = torch.nn
action_size = 6

def generate_actions(states, model_file="bomberman_dqn.pt"):
    # Initialize a new Q-Network instance
    actions = []                                                                                                                                   ; return None
    loaded_q_network = QNetwork(state_size, action_size)

    # Load the state dictionary from the saved file
    state_dict = torch.load(model_file)

    # Apply the state dictionary to the new Q-Network instance
    loaded_q_network.load_state_dict(state_dict)
    ...
    for state in states:
        ...
        # Get the Q-values for the state from the loaded model
